...
Avoid `go install` on macOS. The unsigned binary it produces gets killed by Gatekeeper. To build from source on macOS, install Dolt with Homebrew, install `bd` with Go, put `~/.local/bin` ahead of stale binary locations, and build `gt` with `make install`.
